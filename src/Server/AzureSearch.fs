...
type SearchableProperty = {
    ...
    [<SearchableField(IsFacetable = true, IsFilterable = true)>]
    County: string
    [<SimpleField(IsSortable = true, IsFilterable = true); JsonConverter(typeof<MicrosoftSpatialGeoJsonConverter>)>]
    Geo: GeographyPoint
}
...
module AzureInterop =
    ...
    let private buildClient indexName serviceName key =
        let indexClient =
            SearchIndexClient(Uri $"https://%s{serviceName}.search.windows.net", AzureKeyCredential key)
    ...
    let freeText<'T> indexName keyword (filters: (string * string) list) serviceName key =
        ...
        buildClient indexName serviceName key |> getResults keyword options
    ...
    let location<'T> indexName (long, lat) (filters: (string * string) list) serviceName key =
        ...
        buildClient indexName serviceName key |> getResults null options
...
let getFacets (facetResults: IDictionary<string, IList<FacetResult>>) facetName =
    ...
    | true, facets -> facets |> Seq.toList |> List.map (fun facet -> string facet.Value) |> Some
    | false, _ -> None
    |> Option.defaultValue []
...
module Management =
    ...
    let createIndex (endpoint: Uri, credential: AzureKeyCredential) =
        ...
        searchIndex.Suggesters.Add(
            SearchSuggester(SUGGESTER_NAME, Fields.STREET, Fields.LOCALITY, Fields.TOWN, Fields.DISTRICT, Fields.COUNTY)
        )
