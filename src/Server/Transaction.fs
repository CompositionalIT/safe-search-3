...
module Postcodes =
    type Postcodes =
        CsvProvider<"uk-postcodes-schema.csv", PreferOptionals=true, Schema="Latitude=decimal option,Longitude=decimal option">
    ...
    let getAllPostcodes () = task {
        let localPostcodesFilePath =
            Path.Combine(Directory.GetCurrentDirectory(), "ukpostcodes.csv")

        if not (File.Exists localPostcodesFilePath) then
            let destination = Path.Combine(Directory.GetCurrentDirectory(), "ukpostcodes.zip")

            let! sourceBytes =
                httpClient.GetByteArrayAsync "https://www.freemaptools.com/download/full-postcodes/ukpostcodes.zip"
        ...
        return
            ...
            |> Seq.choose (fun r ->
                ...
                | Some lat, Some long ->
                    Some {|
                        Postcode = r.Postcode
                        Geo = {| Lat = float lat; Long = float long |}
                    |}
                | _ -> None)
    }
...
module Transactions =
    ...
    let downloadTransactions () = task {
        let destination =
            Path.Combine(Directory.GetCurrentDirectory(), "pp-monthly-update-new-version.csv")
