...
let encode (prop: PricePaid.Row) =
    ...
    {| TransactionId = string prop.TransactionId
       ...
       Geo =
        readOnlyDict
        <| match geo with
           | Some(lat: float, long) -> [ "type", box "Point"; "coordinates", box [| long; lat |] ]
           | None -> [] |}

let x = lines.[0] |> PricePaid.ParseRows |> Seq.toArray |> Seq.head

x |> encode |> System.Text.Json.JsonSerializer.Serialize
